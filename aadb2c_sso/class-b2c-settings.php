...
class B2C_Settings {
	...
	function __construct() {
		...
		if (isset($config_elements)) {
			...
			self::$password_reset_policy = $config_elements['b2c_password_reset'];
			self::$sign_up_policy        = $config_elements['b2c_sign_up'];
			self::$admin_policy          = $config_elements['b2c_admin_policy_id'];
			self::$edit_profile_policy   = $config_elements['b2c_edit_profile_policy_id'];
			// self::$redirect_uri          = urlencode(site_url().'/wp-login.php'); 
