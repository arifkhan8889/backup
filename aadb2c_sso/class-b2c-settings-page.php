...
class B2C_Settings_Page
{
    ...
    {        
        ...
        add_settings_field(
         ...
         add_settings_field(
            'b2c_password_reset', // ID
            'Reset Password Policy', // Title 
            array( $this, 'b2c_password_reset_callback' ), // Callback
            'b2c-settings-page', // Page
            'service_config_section' // Section           
        ); 
        add_settings_field(
            'b2c_sign_up', // ID
            'Sign up Policy', // Title 
            array( $this, 'b2c_sign_up_callback' ), // Callback
            'b2c-settings-page', // Page
            'service_config_section' // Section           
        );      
    ...
    public function sanitize( $input )
    {
        ...
        if( isset( $input['b2c_password_reset'] ) )
            $new_input['b2c_password_reset'] = sanitize_text_field(strtolower( $input['b2c_password_reset'] ));

        if( isset( $input['b2c_sign_up'] ) )
            $new_input['b2c_sign_up'] = sanitize_text_field(strtolower( $input['b2c_sign_up'] ));
    ...
    public function b2c_password_reset_callback()
    {
        printf(
            '<input type="text" id="b2c_password_reset" name="b2c_config_elements[b2c_password_reset]" value="%s" />',
            isset( $this->options['b2c_password_reset'] ) ? esc_attr( $this->options['b2c_password_reset']) : ''
        );
    }  

    public function b2c_sign_up_callback(){
        printf(
            '<input type="text" id="b2c_sign_up" name="b2c_config_elements[b2c_sign_up]" value="%s" />',
            isset( $this->options['b2c_sign_up'] ) ? esc_attr( $this->options['b2c_sign_up']) : ''
        );
    }
